...
model = Pix2Pix(0.0002, 0.5, 0.5, test_batch)

checkpoint_callback = ModelCheckpoint(
    monitor='val_loss',
    dirpath='./',
    filename='./checkpoints/gan64-{epoch:02d}-{val_loss:.2f}',
    save_top_k=3,
    mode='min'
)
...
logger = WandbLogger(name = "Colorisation-pix2pix")
trainer = pl.Trainer(
    accelerator="auto",
    max_epochs=90,
    logger = logger,
...
torch.save(model.generator, './checkpoints/generator_gan128_weights')
torch.save(model.discriminator, './checkpoints/discriminator_gan128_weights')